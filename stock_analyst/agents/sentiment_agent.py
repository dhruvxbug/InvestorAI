...
class SentimentAgent:
    ...
    POSITIVE_WORDS = {"upgrade", "beat", "growth", "strong", "outperform", "bullish", "profit rise", "record"}
    NEGATIVE_WORDS = {"downgrade", "miss", "decline", "weak", "probe", "fraud", "loss", "lawsuit", "warning"}

    HIGH_IMPACT_WORDS = {"earnings miss", "ceo change", "cfo change", "regulatory action", "fraud", "ban", "default"}
    MEDIUM_IMPACT_WORDS = {"guidance", "target price", "downgrade", "upgrade", "margin", "acquisition", "results"}
    ...
    def analyze(self, ticker: str, company_name: str, sector_name: str = "Indian equity") -> dict[str, Any]:
        now = datetime.utcnow()
        ...
        searches = {
            ...
            "analyst_ratings": (f"{company_name} analyst rating target price buy sell {current_year}", 10),
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_map = {
                executor.submit(self.exa.search, query=query, num_results=num_results): key
        ...
        for item in raw_results:
            snippet = " ".join([item.get("title") or "", item.get("summary") or "", " ".join(item.get("highlights") or [])]).strip()
            sentiment = self._sentiment(snippet)
            impact = self._impact(snippet)
            weight = self._impact_weight(impact)
        ...
        normalized_score = round((sentiment_score_raw / total_weight) if total_weight else 0.0, 2)
        ...
        avg_target_price = round(sum(all_target_prices) / len(all_target_prices), 2) if all_target_prices else None

        theme_buckets = {
            "Earnings momentum": ["earnings", "results", "profit", "margin", "beat"],
            "Analyst action": ["rating", "target", "upgrade", "downgrade", "brokerage"],
            "Regulatory developments": ["sebi", "rbi", "policy", "regulatory", "compliance"],
            "Growth and expansion": ["growth", "expansion", "capex", "demand", "outlook"],
        ...
        sentiment_themes = [theme for theme, count in theme_counter.most_common(5) if count > 0]

        key_events = [
            article["headline"]
            for article in sorted(articles, key=lambda a: {"HIGH": 3, "MEDIUM": 2, "LOW": 1}[a["impact_level"]], reverse=True)[:5]
        ...
        return {
            ...
            "sentiment_reasoning": (
                ...
                f"Weighted sentiment score={normalized_score}."
            ),
        }
